...
def register():
    ...
    conn = sqlite3.connect('users.db')
    cur = conn.cursor()
    email_exists = cur.execute('SELECT * FROM user WHERE email=?', [email]).fetchone()
    name_exists = cur.execute('SELECT * FROM user WHERE name=?', [name]).fetchone()
    ...
    cur.execute('INSERT INTO user (name, email, password) VALUES (?,?,?)', new_user)
    id = cur.execute('SELECT id FROM user WHERE email=?', [email]).fetchone()
    conn.commit()
    conn.close()
...
def view_user():
    ...
    conn = sqlite3.connect('users.db')
    cur = conn.cursor()
    name = cur.execute('SELECT name FROM user WHERE id=?', [id]).fetchone()
    conn.close()
...
def change_password():
    ...
    conn = sqlite3.connect('users.db')
    cur = conn.cursor()
    cur.execute('UPDATE user SET password=? WHERE email=?', [hash_password(new_password), email])
    conn.commit()
    conn.close()
