...
def post():
    ...
    article_post = [day.strftime("%x %X"), day.strftime("%x %X"), content, title, author]

    conn = sqlite3.connect('articles.db')
    cur = conn.cursor()
    cur.execute('''INSERT INTO article (date_created, date_modified, content, title, author)
                    VALUES (?, ?, ?, ?, ?)''', article_post)
    id = cur.execute('''SELECT id FROM article
...
def view():
    ...
    cur = conn.cursor()
    article_post = cur.execute('SELECT * FROM article WHERE id=?', [articleid]).fetchone()
    conn.close()
...
def edit():
    ...
    if user == author[0]:
        ...
        if title and content:
            ...
            cur.execute('UPDATE article SET date_modified=?, content=?, title=? WHERE id=?', article_update)
        elif title and not content:
            article_update = [day.strftime("%x %X"), title, articleid]
            cur.execute('UPDATE article SET date_modified=?, title=? WHERE id=?', article_update)
        elif content and not title:
            article_update = [day.strftime("%x %X"), content, articleid]
            cur.execute('UPDATE article SET date_modified=?, content=?, WHERE id=?', article_update)
        else:
...
def delete():
    ...
    if user == author[0]:
        cur.execute('DELETE FROM article WHERE id=?', [articleid])
        conn.commit()
        conn.close()
...
def view_all():
    ...
    conn.row_factory = dict_factory
    cur = conn.cursor()
    articles = cur.execute('SELECT * FROM article').fetchall()
    conn.close()
...
def view_recent():
    ...
    conn.row_factory = dict_factory
    cur = conn.cursor()
    articles = cur.execute('SELECT * FROM article ORDER BY date_created DESC LIMIT ?', [num]).fetchall()
    conn.close()
...
def view_meta():
    ...
    conn.row_factory = dict_factory
    cur = conn.cursor()
    articles = cur.execute('SELECT date_created, title, author, id FROM article ORDER BY date_created DESC LIMIT ?', [num]).fetchall()
    conn.close()
